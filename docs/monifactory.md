...
The published serving snapshot lives under `public/datasets/monifactory/` and is tracked in Git and bundled into the Docker image. A fresh clone includes it; the staging inputs above are only needed to regenerate it. See [self-hosting](self-hosting.md) for deployment. Legacy GTNH manifests remain parseable; local server overrides use `DATASET_MANIFEST_PATH`, with the matching browser URL in `NEXT_PUBLIC_DATASET_MANIFEST_URL`.
...
## Healing Hostile Microverse missions

Coverage now reaches **9,753 renewable resources**, with **30 additional healing Hostile Microverse missions** admitted. The source adapter requires an explicit `hostileDecayRate` read from the supplied MoniLabs configuration; omitting the configuration leaves hostile recipes unproven. The copied instance's on-disk value is 10. The generated guide records this scalar and the configuration SHA-256, without publishing the file or its local path. This is a source-code model using the supplied configuration, not an in-game endurance test or a fresh runtime settings export.

```powershell
npm run monifactory:renewables -- ".pipeline/monifactory/0.13.7-expert/catalog.json" ".pipeline/monifactory/0.13.7-expert/guide/full-recipes.json" ".pipeline/monifactory/0.13.7-expert/textures/texture-index.json" "public/datasets/monifactory/renewables.json.gz" "C:\path\to\minecraft\config\monilabs.yaml"
```

MoniLabs' working callback heals these missions according to their negative damage rate; the formed projector also loses integrity on every passive tick, including idle periods. Hostile Microverses cannot use Quantum Flux repair. Transformation from Normal resets integrity to 100,000, while destroying the structure removes the hostile state. Startup lists projection materials and the combat-miner TNT/lava transformation separately from replenished mission ingredients. Ordinary combat miners remain consumed inputs with damaged miners as outputs, so their replacement or repair must be proven. Stabilized miners are guaranteed returned stock.

Each accepted route requires a dedicated projector, one parallel at base voltage, continuous extraction, buffered power and supply, and bounded **total** idle/stalled time per completed mission. The conservative allowance is half the smaller of the mission's net healing budget and the integrity buffer, converted to ticks. At decay 10, a 1,000-tick mission healing 30 per working tick allows 1,000 total idle ticks (50 seconds); a 4,000-tick mission healing 20 allows 2,000 (100 seconds). Several interruptions share this allowance. Equal-healing/decay missions, state transitions, corrupted or shattered state and unsupported extra fields remain unproven. Long outages need projection and transformation again; the guide does not promise recovery without these steps.

Real exported examples verify ordinary versus stabilized returns and fixed mob-drop quantities. Simulations check repeated missions, integrity saturation, both working/decay callback orders, and interruptions split within and between missions. Typecheck and targeted lint passed; all 128 test files passed (1,375 tests plus the existing expected failure), including all 89 Monifactory checks. Browser inspection verified the bone route, explicit timing requirements and reusable miner. A narrow-screen overflow caused by long search-result names was fixed and verified at 390px. Other serializers, custom machines and world/loot interactions remain incomplete.

## Reconstruction and Thermal crop/material recipes

The guide now reaches **9,938 resources**, with 59,067 normalized entries and 48,107 reviewed entries before synthesized loops. Review of the pinned pack's `singleblock_registry.js` showed that the Atomic Reconstructor is an ordinary tiered singleblock, so its blanket exclusion was removed. Rotten flesh to leather, steel to Damascus steel, plant conversions and reconstruction gems now retain their actual input requirements and renewable power dependency. The Quintessence Infuser and Naquadah Fuel Refinery also use ordinary multiblock material recipes; their blanket exclusions were removed, while stateful soul-vial ingredients are still rejected by the NBT guard. These changes affect guide reachability, not board calculation coverage.

The new Thermal adapter admits **64 Phytogenic Insolator and 53 Centrifugal Separator recipes**. It preserves concrete/tag inputs, water, item/fluid outputs and optional catalyst behavior. The two conditional azalea recipes remain excluded pending condition review. The baseline requires no augments and an empty catalyst slot; renewable FE/RF and extraction are explicit. Thermal's 500 mB default water amount and recipe water multiplier are included. An output chance above one means whole guaranteed stacks plus a fractional bonus: the adapter splits these, and only guaranteed returns can restore the starting seed. A less-than-guaranteed seed return cannot establish a renewable crop alone.

Source review used Thermal Expansion 11.0.1 at `fecc6f0d7bf61b18b7e7b959721ee345debc7bcd` and Thermal Core 11.0.6 at `35ec43d9414dfb47e82de9e963ba69c291d8fa86`. The installed Foundation JAR embeds `thermal_core-1.20.1-11.0.6.24.jar`; Expansion is `11.0.1.29`. Relevant classes include `MachineInsolatorBlockEntity`, `InsolatorRecipe`, `InsolatorRecipeManager`, `CatalyzedMachineRecipe` and `MachineBlockEntity.resolveOutputs`. The recipe data still comes from the copied instance's export. This is a source-backed material model, not a machine speed benchmark. Reviewed machine steps now expose source links in the page, including inside recycling batches, and display familiar Thermal machine names.

Verification passed typecheck and targeted ESLint; all 129 test files passed (1,380 tests plus the existing expected failure), including all 94 Monifactory checks. Tests use actual rubber-sapling, pink-petal, coral, muddy-root and reconstruction recipes. They check guaranteed versus bonus yields, water use, seed recycling, power dependency, unsafe fields and conditional recipe rejection. Browser inspection confirmed the brain-coral route, its one-time coral stock, ongoing water/power and source links, without reported browser exceptions. All-resource coverage remains open: matter condensation, additional serializers, finite world sources, custom machines and stateful items still need review.

## AE2 machines and condensation

Coverage now reaches **9,981 resources** with 59,095 normalized entries and 48,135 reviewed entries before synthesized loops. The adapter includes all 23 exported AE2 Inscriber recipes and three Charger recipes, plus two source-backed Matter Condenser recipes. Inscribe mode consumes only the middle ingredient and retains top/bottom presses; press mode consumes all occupied slots. Charger completion timing is random but the material output is guaranteed. Unknown modes, conditional records and stateful outputs remain excluded.

The installed MoniLabs JAR embeds AE2 **15.4.10-cosmolite.36**. Review used its exact tag at `ec84475c886f442e86ca2d759734266907e92537` in `Frontiers-PackForge/Applied-Energistics-2-cosmolite`, rather than assuming upstream AE2 behavior. Relevant classes are `CondenserBlockEntity`, `StorageComponentItem`, `InscriberBlockEntity`, `InscriberRecipeSerializer`, `ChargerBlockEntity` and `AEConfig`. No matching condenser/inscriber/charger mixin was found in the pinned MoniLabs source.

The condenser consumes inserted items as matter, with capacity equal to eight times the inserted storage component's bytes. It consumes no electrical power. The copied instance's AE2 config requires **256 items per matter ball** and **256,000 per singularity**, so the shown cobblestone routes reserve a 1k or 64k ME Storage Component respectively. The user must select the named output mode and extract continuously. The condenser destroys inputs even when output is blocked, so the guide explicitly calls for a dedicated renewable cobblestone feed. Condenser recipes are added only when an AE2 configuration is supplied; unsupported capacities fail validation. The generated artifact records the selected values and the configuration hash, without copying the file or local path.

```powershell
npm run monifactory:renewables -- ".pipeline/monifactory/0.13.7-expert/catalog.json" ".pipeline/monifactory/0.13.7-expert/guide/full-recipes.json" ".pipeline/monifactory/0.13.7-expert/textures/texture-index.json" "public/datasets/monifactory/renewables.json.gz" "C:\path\to\minecraft\config\monilabs.yaml" "C:\path\to\minecraft\config\ae2\common.json"
```

The search and route display now show an EU tier only if the selected production proof contains an actual EU-consuming recipe. Thermal, AE2-only and condenser routes no longer acquire a fictitious ULV label from a zero used internally for sorting. Real ULV recipes and EU consumption inside recycling batches retain their tier. Initial machine progression and power-system requirements remain separate.

Typecheck, targeted lint, all 130 test files (1,385 passing tests plus the existing expected failure), and all 99 Monifactory checks passed. Tests cover configured condenser costs/capacities, replenishable feed, reusable versus consumed presses, charger guarantees, invalid inputs and EU labels including batch members. Browser verification confirmed singularity quantity, component, output-mode instructions, source links and the corrected coral-route tier, without reported exceptions. Additional serializers, world interactions and custom/stateful machines remain unfinished.
