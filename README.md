...
- **39,396 native recipes** represented by **45,366 entries across 70 maps**, with machine selection and runtime textures. Includes EBFs, all 128 Greenhouse recipes, assembly lines, distillation towers, large and mega machines, Microverse missions and fuel generation. See [recipe coverage](docs/recipe-coverage.md) for calculation status.
...
Ordinary-machine calculations were checked against **18,480 live modifier reference cases** and **39,098 live inventory-matching cases**. Native recipe hashes also match the imported values. GTNH-specific bonuses and automatic container conversions are disabled for Monifactory recipes.

The expanded multiblock support is **source-derived and unverified**. It includes previously excluded conditions, NBT-specific items and random quantities, preserving native details in recipe metadata and notes. Structure requirements, research unlocks, dimensions and other prerequisites must be met in game. New adapters assume stocked inputs, free output space and batch mode off; they have not received the live checks listed below.
...
Power planning supports native **LV–HV combustion generators, gas turbines and steam turbines**, checked against **110 runtime cases**. Use **Power** above the resource search to select a fuel recipe, choose the generator tier on its card, and put an EU output target in Solve or Pool. Connect the fuel input to its production chain, or add that chain in Pool mode. Check **Net EU target** on the EU product to reserve the power consumed by every active processing machine on the plan. The target tooltip shows gross generation, plan power use and net available EU/t. Only one target reserves the plan's consumption.

[Download an ethanol example](public/examples/monifactory-ethanol-power.json) and import it through the plan menu. It sizes the complete sugar cane → biomass → ethanol → combustion chain for 128 net EU/t, with water and initial seeds supplied externally. The unboosted crop route is deliberately simple and inefficient: it illustrates why gross generator output alone is insufficient. Card counts show continuous equivalent capacity; the machine list rounds up to whole machines and separates peak from average power. Buffer production between machines. Large engines, rotor turbines, boilers and naquadah reactors now have source-derived adapters; their new rates are unverified. Turbines expose combined rotor/holder statistics, engines expose oxygen boost, and boilers expose throttle. The inherited GTNH generator tables are not used for Monifactory.

The renewable guide covers more recipe types than the board calculator. A guide route establishes material availability under its listed assumptions; it does not establish machine speed, power sizing or full board support for that machine. World-state extraction machines, additional recipe serializers and exact stateful behavior still need further work. Chance outputs use long-run expectations, not guaranteed short-term production.
